...
def getMethods():
    ...
    class Method(object):
        ...
        CATEGORY_DICT = {0:'experiment', 1:'observed', 2:'empirical', 3:'theory', 4:'semiempirical', 5:'compilation', 6:'ritz'}
        DESC_DICT = {0: "VALD exp - experimentally measured value",
                     1: "VALD obs - value known from observation, usually astronomical",
                     2: "VALD emp - relativistic Hartree-Fock calculations, normalized to the experimental lifetimes",
                     3: "VALD pred - predicted value from theoretical calculations",
                     4: "VALD calc - relativistic Hartree-Fock calculations of lifetimes and transition probabilities",
                     5: "VALD mix - compilation of data",
                     6: "VALD ritz - wavelelength from the upper and lower state energies"}
        def __init__(self, ID):
            self.id = ID
            self.category = self.CATEGORY_DICT[ID]
            self.description = self.DESC_DICT[ID]
    return (Method(0), Method(1), Method(2), Method(3), Method(4), Method(5), Method(6))
