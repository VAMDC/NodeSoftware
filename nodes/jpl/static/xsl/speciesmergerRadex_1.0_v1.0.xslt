...
xmlns:xsl ="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common" xmlns:xsams10="http://vamdc.org/xml/xsams/1.0" xmlns:xsams03="http://vamdc.org/xml/xsams/0.3">

<xsl:key name="molecule_by_inchikey" match="/xsams10:XSAMSData/xsams10:Species/xsams10:Molecules/xsams10:Molecule" use="/xsams10:MolecularChemicalSpecies/xsams10:InChIKey"/>

<xsl:key name="molstate" match="//xsams10:MolecularState" use="@stateID"/>
...
<xsl:key name="collision" match="xsams10:CollisionalTransition" use="xsams10:Reactant/xsams10:SpeciesRef"/>
...
<xsl:param name="colfile" />
<xsl:template match="/xsams10:XSAMSData">

<!-- <xsl:text>S2</xsl:text>
<xsl:value-of select="$species2"/>
<xsl:text>COLFILE</xsl:text>
<xsl:value-of select="document($colfile,/*)//xsams10:Molecule[@speciesID=$species2]/@speciesID"/>
-->
   ...
   <!-- filter molecules -->
   <xsl:variable name="molecule1" select="//xsams10:Molecule[@speciesID=$species1][1]"/>
   
   <!-- or atoms -->
   <xsl:variable name="atom1" select="//xsams10:Atom[@speciesID=$species1][1]"/>
   ...
   <xsl:variable name="transitions1_rtf">
      <xsl:for-each select="//xsams10:RadiativeTransition">
        <xsl:choose>
          <!-- filter via SpeciesRef if specified -->
          <xsl:when test="xsams10:SpeciesRef"> 
             <xsl:if test="xsams10:SpeciesRef=$species1">
               <xsl:copy-of select="."/>
             </xsl:if>
          </xsl:when>
   ...
   <xsl:variable name="transitions1" select="exsl:node-set($transitions1_rtf)"/>
   ...
   <xsl:otherwise>
   <xsl:message>
      <xsl:text />Merging input with '<xsl:value-of select="string($file2)" />
      <xsl:text>'</xsl:text>
   </xsl:message>
   ...
   <xsl:if test="string($species2)=''">
      ...
      <xsl:for-each select="document($colfile,/*)//xsams10:Molecule">
        <xsl:message>
          <xsl:value-of select="@speciesID"/>:
          <xsl:value-of select="xsams10:MolecularChemicalSpecies/xsams10:StoichiometricFormula"/>
        </xsl:message>
      </xsl:for-each>   
      <xsl:message terminate="yes"/>
   </xsl:if>
   ...
   <xsl:variable name="nodes2" select="document($colfile,/*)/node()" />
   ...
   <xsl:variable name="mstates" select="exsl:node-set($mstates_rtf)"/>
   ...
   <xsl:variable name="collisions_rtf">
     <!-- for-each is used to make key working (environment has to be nodes2) -->
     <xsl:for-each select="$nodes2/xsams10:Species/xsams10:Molecules/xsams10:Molecule[@speciesID=$species2]">
         ...
         <xsl:copy>
           ...
           <xsl:for-each select="*|@*">
             <xsl:choose>
               ...
               <xsl:when test="(name()='Reactant' or name()='Product') and xsams10:StateRef">
                 <!-- get new state id from the replacement list-->
                 <xsl:variable name="stateid">
                   <xsl:call-template name="replace_stateid">
                     <xsl:with-param name="id" select="xsams10:StateRef" />
                 ...
                 <xsl:choose>
                   ...
                   <xsl:otherwise>
                      <xsl:copy>
                        <xsl:element name="xsams10:StateRef" ><xsl:value-of select="$position_map/*[@id=$stateid]"/></xsl:element>
                        <xsl:element name="xsams10:SpeciesRef" ><xsl:value-of select="$species1"/></xsl:element>
                     </xsl:copy>                        
                   </xsl:otherwise>
                 </xsl:choose>
   ...
   <xsl:variable name="collisions" select="exsl:node-set($collisions_rtf)"/>
   ...
   <xsl:if test="count($collisions/*)>0">
     ...
     <xsl:variable name="col_partners" select="$collisions/*[1]/xsams10:Reactant"/>
<!--      
     <xsl:variable name="col_temps" >
       <xsl:for-each select="$collisions/*[1]">
         <xsl:value-of select="xsams10:DataSets/xsams10:DataSet[@dataDescription='rateCoefficient']/xsams10:TabulatedData/xsams10:X[@units='K']/xsams10:DataList"/>
       </xsl:for-each>
     </xsl:variable>
 -->
     ...
     <xsl:call-template name="radexoutput-collision-header" >
       <xsl:with-param name="num_col_partners" select="count($col_partners)"/> <!--collisions/*[1]/xsams10:Reactant)"/>-->
       <xsl:with-param name="partners" select="$col_partners"/>
       <xsl:with-param name="comments" select="$collisions/*[1]/xsams10:Comments"/>
...
<!-- TEMPLATES FOR RADEX - OUTPUT -->
  ...
  <xsl:template name="radexoutput-molecule">
    ...
    <xsl:text>!MOLECULE&#xa;</xsl:text>
    <xsl:value-of select="$molecule/xsams10:MolecularChemicalSpecies/xsams10:StoichiometricFormula"/>
    <xsl:text>&#xa;</xsl:text>
...
<!--     <xsl:value-of select="substring-after(@stateID,'-')"/>
     <xsl:text>    </xsl:text>-->
     <xsl:value-of select="format-number(xsams10:MolecularStateCharacterisation/xsams10:StateEnergy/xsams10:Value,'000000000000.00000000 ')"/>
     <xsl:text>    </xsl:text>
     <xsl:value-of select="format-number(xsams10:MolecularStateCharacterisation/xsams10:TotalStatisticalWeight,' 0000', 'example')"/>
     <xsl:text>&#xa;</xsl:text>
  </xsl:template>
...
<!--       <xsl:value-of select="substring-after(xsams10:LowerStateRef,'-')"/>
  ...
  <xsl:template name="radexoutput-collision-header" >        
    ...
    <xsl:text>&#xa;</xsl:text>
    <xsl:for-each select="$partners/xsams10:SpeciesRef">
      <xsl:value-of select="."/>
      <xsl:text>   </xsl:text>
    </xsl:for-each>
...
<!--    <xsl:text>  </xsl:text>
  ...
  <xsl:template name="radexoutput-collision" match="xsams10:CollisionalTransition"> 
      <xsl:param name="speciesid"/>       
      <xsl:value-of select="position()"/>
      <xsl:text>  </xsl:text>
      <xsl:value-of select="xsams10:Reactant[xsams10:SpeciesRef=$speciesid]/xsams10:StateRef"/>
      <xsl:text>  </xsl:text>
...
<xsl:template name="compare-mols">
  ...
  <xsl:param name="mol2" />
  <xsl:choose>
  <xsl:when test="$mol1/xsams10:MolecularChemicalSpecies/xsams10:InChIKey = $mol2/xsams10:MolecularChemicalSpecies/xsams10:InChIKey">=</xsl:when>
  <xsl:otherwise>!</xsl:otherwise>
  </xsl:choose>
</xsl:template>
  ...
  <xsl:param name="state2" />
   ...
   <xsl:variable name="diff-qns">
     <xsl:call-template name="is_qns1_in_qns2">
       <xsl:with-param name="qns1" select="$state1/xsams10:Case/*/*" />
       <xsl:with-param name="qns2" select="$state2/xsams10:Case/*/*" />
     </xsl:call-template>
   </xsl:variable>
