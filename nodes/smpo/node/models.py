...
class Isotopes2(models.Model):
    ...
    w2 = models.FloatField(null=True, blank=True)
    w3 = models.FloatField(null=True, blank=True)
    wh1 = models.FloatField(null=True, blank=True)
    wh2 = models.FloatField(null=True, blank=True)
    wh3 = models.FloatField(null=True, blank=True)
    s1 = models.CharField(max_length=6, null=True, blank=True)
    s2 = models.CharField(max_length=6, null=True, blank=True)
    s3 = models.CharField(max_length=6, null=True, blank=True)
    ref_mass = models.IntegerField(null=True, blank=True)
    ref_abundance = models.IntegerField(null=True, blank=True)
    ref_abc = models.IntegerField(db_column='ref_ABC', null=True, blank=True) 
    ref_w = models.IntegerField(null=True, blank=True)
    ref_wh = models.IntegerField(null=True, blank=True)
    class Meta:
        db_table = u'Isotopes2'
#        unique_together = ('mol_id','mi_id')
...
class Transitions(models.Model):
    mol_id = models.IntegerField(primary_key=True)
    isotid = models.ForeignKey(Isotopes, db_column='mi_id')
    mi_id = models.IntegerField()
    inchi = models.CharField(max_length=75, db_column='InChi') 
    inchikey = models.CharField(max_length=75, db_column='InChiKey') 
    nnn = models.CharField(max_length=6)
    vlup = models.CharField(db_column='VLup',max_length=6) 
    vllow = models.CharField(db_column='VLlow',max_length=6) 
    ...
    eup = models.FloatField(null=True, db_column='Eup', blank=True) 
    ...
    ref_wn = models.IntegerField(null=True, blank=True, db_column='rWN')
    ref_s = models.IntegerField(null=True, blank=True, db_column='rS')
    ref_e = models.IntegerField(null=True, blank=True, db_column='rElow')
    class Meta:
        db_table = u'transitions_vln'
        unique_together = ('mol_id','mi_id','vlup','vllow','jup','kaup','kcup','jlow','kalow','kclow')

    def getTranId(self):
        return str(self.mol_id) + '-' + str(self.mi_id) + '-' + str(self.vlup) + '-' + str(self.jup) + '-' + str(self.kaup) + '-' + str(self.kcup) + '-' + self.vllow + '-' + str(self.jlow) + '-' + str(self.kalow) + '-' + str(self.kclow)

    def getStateRefUp(self):
        return str(self.mol_id) + '-' + str(self.mi_id) + '-' + str(self.vlup) + '-' + self.jup + '-' + self.kaup + '-' + self.kcup

    def getStateRefLow(self):
        return str(self.mol_id) + '-' + str(self.mi_id) + '-' + str(self.vllow) + '-' + self.jlow + '-' + self.kalow + '-' + self.kclow
...
class State(object):
    def __init__(self, arg, val):
        self.id = str(arg[0]) + '-' + str(arg[1]) + '-' + str(arg[2]) + '-' + str(arg[3]) + '-' + str(arg[4]) + '-' + str(arg[5])
        self.energy = val
        vvv = str(arg[2])
        self.v1 = vvv[0]
        self.v2 = vvv[1]
