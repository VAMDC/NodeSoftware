...
<xsl:value-of select="document($colfile,/*)//xsams03:Molecule[@speciesID=$species2]/@speciesID"/>
-->
   ...
   <!-- filter molecules -->
   <xsl:variable name="molecule1" select="//xsams03:Molecule[@speciesID=$species1][1]"/>
   
   <!-- or atoms -->
   <xsl:variable name="atom1" select="//xsams03:Atom[@speciesID=$species1][1]"/>
   ...
   <xsl:if test="string($species2)=''">
      ...
      <xsl:for-each select="document($colfile,/*)//xsams03:Molecule">
        <xsl:message>
   ...
   <xsl:if test="count($collisions/*)>0">
     ...
     <xsl:variable name="col_partners" select="$collisions/*[1]/xsams03:Reactant"/>
<!--      
 ...
 -->
     <xsl:variable name="col_temps" select="$collisions/*[1]/xsams03:DataSets/xsams03:DataSet[@dataDescription='rateCoefficient']/xsams03:TabulatedData/xsams03:X[@units='K']/xsams03:DataList"/>
...
<!-- TEMPLATES FOR RADEX - OUTPUT -->
  ...
  <xsl:template name="radexoutput-molecule">
    ...
    <xsl:text>!MOLECULE&#xa;</xsl:text>
    <xsl:value-of select="$molecule/xsams03:MolecularChemicalSpecies/xsams03:StoichiometricFormula"/>
    <xsl:text>&#xa;</xsl:text>
    <xsl:text>!MOLECULAR WEIGHT&#xa;</xsl:text>
    <xsl:value-of select="$molecule/xsams03:MolecularChemicalSpecies/xsams03:StableMolecularProperties/xsams03:MolecularWeight/xsams03:Value"/>
    <xsl:text>&#xa;</xsl:text>
    <xsl:text>!NUMBER OF ENERGY LEVELS&#xa;</xsl:text>
    <xsl:value-of select = "count($molecule/xsams03:MolecularState)" /> 
    <xsl:text>&#xa;</xsl:text>
    <xsl:text>!LEVEL + ENERGIES(cm^-1) + WEIGHT + J&#xa;</xsl:text>
        <xsl:apply-templates select="$molecule/xsams03:MolecularState"/>
